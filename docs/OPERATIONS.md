...
For a production release, back up Supabase, apply migrations through `009_canonical_room_type_compatibility.sql` in numeric order (skip ones already recorded as applied), then execute `notify pgrst, 'reload schema';`. No new migration accompanies the payment-contract repair. Deploy the committed backend to Render, confirm `/health`, `/health/schema`, and `/rooms`, and only then publish the frontend cache-busted script. The frontend must use the configured Render HTTPS base URL, send JSON with `Content-Type: application/json`, show `response.message` (and field messages from `response.errors`) for expected HTTP failures, and reserve its generic fallback for network failures or malformed/unexpected server responses.
