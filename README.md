...
Email is optional and provider-neutral: configure `EMAIL_WEBHOOK_URL`, its bearer `EMAIL_WEBHOOK_TOKEN`, sender/admin addresses, and guest-house contact fields. On a verified/confirmed booking, the server posts one `booking.confirmed` JSON event containing separate `guest` and `admin` objects. The provider should render/send both messages. Booking success is not rolled back when notification delivery fails, and `email_sent_at` prevents successful deliveries being repeated. A failed delivery is logged and may be retried by an operations job.
...
Exchange the bootstrap key with `POST /admin/login`, either as `X-Admin-Key` or `{"admin_key":"..."}`. The response is a one-hour, HMAC-SHA256 signed bearer token. Send it as `Authorization: Bearer <token>` to:
